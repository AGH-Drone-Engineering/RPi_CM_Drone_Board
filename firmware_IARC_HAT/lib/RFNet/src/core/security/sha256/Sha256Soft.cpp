#include "Sha256Soft.h"
#include <string.h>

static const uint32_t K[64] = {
    0x428a2f98,0x71374491,0xb5c0fbcf,0xe9b5dba5,0x3956c25b,0x59f111f1,0x923f82a4,0xab1c5ed5,
    0xd807aa98,0x12835b01,0x243185be,0x550c7dc3,0x72be5d74,0x80deb1fe,0x9bdc06a7,0xc19bf174,
    0xe49b69c1,0xefbe4786,0x0fc19dc6,0x240ca1cc,0x2de92c6f,0x4a7484aa,0x5cb0a9dc,0x76f988da,
    0x983e5152,0xa831c66d,0xb00327c8,0xbf597fc7,0xc6e00bf3,0xd5a79147,0x06ca6351,0x14292967,
    0x27b70a85,0x2e1b2138,0x4d2c6dfc,0x53380d13,0x650a7354,0x766a0abb,0x81c2c92e,0x92722c85,
    0xa2bfe8a1,0xa81a664b,0xc24b8b70,0xc76c51a3,0xd192e819,0xd6990624,0xf40e3585,0x106aa070,
    0x19a4c116,0x1e376c08,0x2748774c,0x34b0bcb5,0x391c0cb3,0x4ed8aa4a,0x5b9cca4f,0x682e6ff3,
    0x748f82ee,0x78a5636f,0x84c87814,0x8cc70208,0x90befffa,0xa4506ceb,0xbef9a3f7,0xc67178f2
};

#define ROR32(x,n) (((x)>>(n))|((x)<<(32-(n))))
#define CH(x,y,z)  (((x)&(y))^(~(x)&(z)))
#define MAJ(x,y,z) (((x)&(y))^((x)&(z))^((y)&(z)))
#define EP0(x)     (ROR32(x,2)^ROR32(x,13)^ROR32(x,22))
#define EP1(x)     (ROR32(x,6)^ROR32(x,11)^ROR32(x,25))
#define SIG0(x)    (ROR32(x,7)^ROR32(x,18)^((x)>>3))
#define SIG1(x)    (ROR32(x,17)^ROR32(x,19)^((x)>>10))

static void sha256_block(const uint8_t block[64], uint32_t h[8]) {
    uint32_t w[64];
    for (int i = 0; i < 16; i++) {
        w[i] = ((uint32_t)block[i*4+0]<<24)|((uint32_t)block[i*4+1]<<16)|
               ((uint32_t)block[i*4+2]<< 8)| (uint32_t)block[i*4+3];
    }
    for (int i = 16; i < 64; i++)
        w[i] = SIG1(w[i-2]) + w[i-7] + SIG0(w[i-15]) + w[i-16];

    uint32_t a=h[0],b=h[1],c=h[2],d=h[3],e=h[4],f=h[5],g=h[6],hh=h[7];
    for (int i = 0; i < 64; i++) {
        uint32_t t1 = hh + EP1(e) + CH(e,f,g) + K[i] + w[i];
        uint32_t t2 = EP0(a) + MAJ(a,b,c);
        hh=g; g=f; f=e; e=d+t1; d=c; c=b; b=a; a=t1+t2;
    }
    h[0]+=a; h[1]+=b; h[2]+=c; h[3]+=d;
    h[4]+=e; h[5]+=f; h[6]+=g; h[7]+=hh;
}

void sha256_init(Sha256Ctx& ctx) {
    ctx.h[0]=0x6a09e667; ctx.h[1]=0xbb67ae85; ctx.h[2]=0x3c6ef372; ctx.h[3]=0xa54ff53a;
    ctx.h[4]=0x510e527f; ctx.h[5]=0x9b05688c; ctx.h[6]=0x1f83d9ab; ctx.h[7]=0x5be0cd19;
    ctx.totalLen = 0;
    ctx.bufLen   = 0;
}

void sha256_update(Sha256Ctx& ctx, const uint8_t* data, size_t len) {
    ctx.totalLen += len;

    if (ctx.bufLen > 0) {
        uint32_t need = 64 - ctx.bufLen;
        if (len < need) {
            memcpy(ctx.buf + ctx.bufLen, data, len);
            ctx.bufLen += (uint32_t)len;
            return;
        }
        memcpy(ctx.buf + ctx.bufLen, data, need);
        sha256_block(ctx.buf, ctx.h);
        data    += need;
        len     -= need;
        ctx.bufLen = 0;
    }

    while (len >= 64) {
        sha256_block(data, ctx.h);
        data += 64;
        len  -= 64;
    }

    if (len > 0) {
        memcpy(ctx.buf, data, len);
        ctx.bufLen = (uint32_t)len;
    }
}

void sha256_final(Sha256Ctx& ctx, uint8_t out[32]) {
    uint64_t bitLen = ctx.totalLen * 8;

    ctx.buf[ctx.bufLen++] = 0x80;
    if (ctx.bufLen > 56) {
        // No room left for the 8-byte length; use an extra block.
        memset(ctx.buf + ctx.bufLen, 0, 64 - ctx.bufLen);
        sha256_block(ctx.buf, ctx.h);
        ctx.bufLen = 0;
    }
    memset(ctx.buf + ctx.bufLen, 0, 56 - ctx.bufLen);

    for (int i = 7; i >= 0; --i) {
        ctx.buf[56 + (7 - i)] = (uint8_t)((bitLen >> (i * 8)) & 0xFF);
    }
    sha256_block(ctx.buf, ctx.h);

    for (int i = 0; i < 8; ++i) {
        out[i*4+0]=(uint8_t)(ctx.h[i]>>24); out[i*4+1]=(uint8_t)(ctx.h[i]>>16);
        out[i*4+2]=(uint8_t)(ctx.h[i]>> 8); out[i*4+3]=(uint8_t)(ctx.h[i]);
    }
}

void sha256_compute(const uint8_t* data, size_t len, uint8_t out[32]) {
    Sha256Ctx ctx;
    sha256_init(ctx);
    sha256_update(ctx, data, len);
    sha256_final(ctx, out);
}
